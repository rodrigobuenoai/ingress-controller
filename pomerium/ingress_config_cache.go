...
// reconcileInitial updates Ingress status after applying the full config, and
// those updates must not trigger another full config rebuild.
...
func fingerprintIngressConfig(ic *model.IngressConfig) (ingressConfigFingerprint, error) {
	...
	for _, service := range ic.Services {
		input.Services = append(input.Services, serviceFingerprint{
			Object: objectMetaFingerprint(service.ObjectMeta),
			Spec:   service.Spec,
		})
	}
